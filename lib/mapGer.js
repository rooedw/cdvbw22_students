...
async function loadMovement() {
  let mData = await d3.json("data/germany/most_movement.json");
  let dataArray = [];
  for (const key of Object.keys(mData)) {
    if (mData[key].places && mData[key].places != "None") {
      dataArray.push({
        name: key,
        places: mData[key].places.replaceAll(/[\[\]\(\)"' ]/gi, "").split(","),
      });
    }
  }
  dataArray.sort((x, y) => y.places.length - x.places.length);
  dataArray = dataArray.slice(0, 20);
  const offset = 0.02
  dataArray = dataArray.map((x) => {
    return {
      type: "LineString",
      coordinates: x.places.filter((_, i) => i % 3 == 2).map(placeName => geomap[placeName]).filter(x => x).map((y, i) => [parseFloat(y.lon) + i * offset, parseFloat(y.lat) + i * offset]),
      ...x,
    };
  });
